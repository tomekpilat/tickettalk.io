...
Use the deployment runbook for normal deploys, rollback, and key rotation. After any rotation, verify liveness, readiness, registration-free room creation, anonymous join through the exact UUID URL, Realtime updates, and export before revoking the old key. A Supabase service-role key, `JIRA_ENCRYPTION_KEY`, and `JIRA_OAUTH_CLIENT_SECRET` must exist only in the API service runtime environment. Rotate the Atlassian client secret in the developer console and Coolify together, then test a fresh consent and an existing connection refresh before revoking the old secret. Do not rotate the Jira encryption key in place: first implement a dual-key re-encryption migration or disconnect and reconnect every affected room.
