...
describe('API client', () => {
  it('maps every JSON endpoint to its HTTP contract', async () => {
    ...
    const calls = [
      ...
      [() => api.importJira(roomId, 'project = PAY', 'replace'), `/api/rooms/${roomId}/jira/import`, 'POST'],
      [() => api.jiraAssignees(roomId, ticketId, 'Maya Chen'), `/api/rooms/${roomId}/tickets/${ticketId}/jira-assignees?q=Maya%20Chen`, 'GET'],
      [() => api.setJiraAssignee(roomId, ticketId, 'account-1', 'Maya'), `/api/rooms/${roomId}/tickets/${ticketId}/jira-assignee`, 'PUT'],
      [() => api.writebackJira(roomId), `/api/rooms/${roomId}/jira/writeback`, 'POST'],
